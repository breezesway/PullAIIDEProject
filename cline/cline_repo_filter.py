...
def filter_csv_by_keywords():
    ...
    keywords = [
        "Cline IDE", "Cline AI", 
        "using Cline", "Cline Agent", "use Cline", "with Cline", "by Cline", "in Cline", "through Cline", "via Cline",
        "claude", "sonnet 3.7", "deepseek", 
        "制作", "实现", "编写", "生成", "创建", "开发", 
        "built", "build", 
        "基于Cline", "通过Cline", "借助Cline", "用cline", "由cline"
    ]
    
    # Define the exclusion keywords
    exclusion_keywords = [
        "test", "demo", "learn", "practice", "practical", "rule", "rules", "mouse", "pagination", 
        "a cline", "重置", "无限试用", "curse", "3D cline", "your cline", 
        "custom cline", "教程", "cline navigation", "navigation", "cline move", "学习", 
        ...
        "oracle", "store procedure", "mysql", "sql server", "cline library", "Drawing", "draw", 
        "prompt", "collection", "hand gesture", "canvas", "click", "cline array", "SQL", 
        "experiment", "scrollbar", "Portfolio", "template", "course", "tutorial", "toy", "玩具", 
        "实验", "live cline", "guideline", "quiz", "small", "exploring", "realtime", "real-time"
    ]
    ...
    input_file = os.path.join(script_dir, "cline_all_repo.xlsx")
    
    # Check if the file exists
    if not os.path.exists(input_file):
        print(f"Error: File 'filtered_cline_repo.xlsx' not found!")
