...
class GitHubSearch:
    ...
    def _save_search_results(self, search_type: str, repos_dict: Dict[str, Dict], timestamp: str = None) -> None:
        ...
        csv_filename = os.path.join(self.output_dir, f'pearai_{search_type}_{timestamp}.csv')
    ...
    def search_pearai_files(self, is_current_search: bool = True) -> None:
        """Search for repositories containing Pear AI files."""
        pearai_files = [
            "path:.pearai extension:mdc",
            "filename:.pearairules",
            "filename:.pearaiignore",
            "filename:.pearaiindexingignore"
        ]
        
        for file_name in pearai_files:
            query = f"{file_name}"
            url = f"{self.base_url}/search/code"
            params = {
                'q': query,
            ...
            for item in data['items']:
                self._add_repository(item, f"pearai_config: {file_name}", is_current_search)
    ...
    def save_results(self) -> None:
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        csv_filename = os.path.join(self.output_dir, f'pearai_all_repositories_{timestamp}.csv')
...
def main():
    ...
    # PearAI-related keywords
    pearai_keywords = [
        "generated by PearAI",
        "edited using PearAI",
        "developed with PearAI",
        "powered by PearAI",
        "AI coding with PearAI",
        "PearAI suggestion",
        "applied PearAI edit",
        "auto refactor by PearAI",
        "refactored with PearAI",
        "PearAI-assisted change",
        "accept suggestion from PearAI",
        "used PearAI for this part",
        "Generated using Anysphere PearAI",
        "Auto-generated with PearAI AI"
    ]
    ...
    print("Starting search for PearAI-related repositories...")
    
    # Search repositories by description
    searcher.search_repositories_by_description("PearAI")
    ...
    for keyword in pearai_keywords:
    ...
    # Search for PearAI configuration files
    searcher.search_pearai_files()
    searcher._save_search_results("pearai_files", searcher.current_search_repos)
    searcher.current_search_repos.clear()
