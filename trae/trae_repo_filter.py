...
def filter_csv_by_keywords():
    ...
    keywords = [
        "Trae IDE", "Trae AI", 
        "using Trae", "Trae Agent", "use Trae", "with Trae", "by Trae", "in Trae", "through Trae", "via Trae",
        "claude", "sonnet 3.7", "deepseek", 
        "制作", "实现", "编写", "生成", "创建", "开发", 
        "built", "build", 
        "基于Trae", "通过Trae", "借助Trae", "用trae", "由trae"
    ]
    
    # Define the exclusion keywords
    exclusion_keywords = [
        "test", "demo", "learn", "practice", "practical", "rule", "rules", "mouse", "pagination", 
        "a Trae", "重置", "无限试用", "curse", "3D Trae", "your Trae", 
        "custom Trae", "教程", "Trae navigation", "navigation", "Trae move", "学习", 
        ...
        "oracle", "store procedure", "mysql", "sql server", "Trae library", "Drawing", "draw", 
        "prompt", "collection", "hand gesture", "canvas", "click", "Trae array", "SQL", 
    ...
    input_file = os.path.join(script_dir, "trae_all_repo.xlsx")
    
    # Check if the file exists
    if not os.path.exists(input_file):
        print(f"Error: File 'trae_all_repo.xlsx' not found!")
