...
def filter_csv_by_keywords():
    ...
    keywords = [
        "V0 IDE", "V0 AI", 
        "using V0", "V0 Agent", "use V0", "with V0", "by V0", "in V0", "through V0", "via V0",
        "claude", "sonnet 3.7", "deepseek", 
        "制作", "实现", "编写", "生成", "创建", "开发", 
        "built", "build", 
        "基于V0", "通过V0", "借助V0", "用v0", "由v0"
    ]
    
    # Define the exclusion keywords
    exclusion_keywords = [
        "test", "demo", "learn", "practice", "practical", "rule", "rules", "mouse", "pagination", 
        "a v0", "重置", "无限试用", "curse", "3D v0", "your v0", 
        "custom v0", "教程", "v0 navigation", "navigation", "v0 move", "学习", 
        ...
        "oracle", "store procedure", "mysql", "sql server", "v0 library", "Drawing", "draw", 
        "prompt", "collection", "hand gesture", "canvas", "click", "v0 array", "SQL", 
        "experiment", "scrollbar", "Portfolio", "template", "course", "tutorial", "toy", "玩具", 
        "实验", "live v0", "guideline", "quiz", "small", "exploring", "realtime", "real-time"
    ]
    ...
    input_file = os.path.join(script_dir, "v0_all_repo.xlsx")
    
    # Check if the file exists
    if not os.path.exists(input_file):
        print(f"Error: File 'filtered_v0_repo.xlsx' not found!")
