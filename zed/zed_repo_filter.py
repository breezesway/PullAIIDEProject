...
def filter_csv_by_keywords():
    ...
    keywords = [
        "ZED IDE", "ZED AI", 
        "using ZED", "ZED Agent", "use ZED", "with ZED", "by ZED", "in ZED", "through ZED", "via ZED",
        "claude", "sonnet 3.7", "deepseek", 
        "制作", "实现", "编写", "生成", "创建", "开发", 
        "built", "build", 
        "基于ZED", "通过ZED", "借助ZED", "用zed", "由zed"
    ]
    
    # Define the exclusion keywords
    exclusion_keywords = [
        "test", "demo", "learn", "practice", "practical", "rule", "rules", "mouse", "pagination", 
        "a zed", "重置", "无限试用", "curse", "3D zed", "your zed", 
        "custom zed", "教程", "zed navigation", "navigation", "zed move", "学习", 
        ...
        "oracle", "store procedure", "mysql", "sql server", "zed library", "Drawing", "draw", 
        "prompt", "collection", "hand gesture", "canvas", "click", "zed array", "SQL", 
        "experiment", "scrollbar", "Portfolio", "template", "course", "tutorial", "toy", "玩具", 
        "实验", "live zed", "guideline", "quiz", "small", "exploring", "realtime", "real-time"
    ]
    ...
    input_file = os.path.join(script_dir, "zed_all_repo.xlsx")
    
    # Check if the file exists
    if not os.path.exists(input_file):
        print(f"Error: File 'filtered_zed_repo.xlsx' not found!")
